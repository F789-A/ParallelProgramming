...
5. Каждый кадр загружает текстуру на gpu и выводит на экран (средствами OpenGL).
  ...
  Между главным и рендер потоком данные синхронизировались с использованием мьютексов, это место яаляется потенциальным бутылочным горлышком, поэтому была попытка максимально его оптимизировать. В фреймворке Qt для этих целей в середине кадра для синхронизации останавливаются оба потока. 
...
Сложность обработки изображения (рейстрейсинга) в худшем случае: O(w * h * sqrt( x^2 + y^2 + z^2)), где w, h - ширина, высота окна; x, y, z - ширина, высота, глубина отрисовываемого массива.
...
Общее время работы всего потока рендера так же не стал приводить, только время работы непосредственно рейтрейсинга (область, где работа параллелится).
...
Для себя узнал, что все OpenGL объекты должны создаваться в одном потоке (контекст, шейдеры, буфферы и т.д.) (в связи с этим некоторые архитектурные решения оказались неудачными, особенно некрасив код связанный с вводом).

## ???
![Image alt](https://github.com/F789-A/ParallelProgramming/blob/Additional/picture.png)
